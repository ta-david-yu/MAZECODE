...
public class EasingFunction
{
    ...
    public static float EaseOutExpo(float start, float end, float value)
    {
        ...
        // y = (end - start) * (-Mathf.Pow(2, -10 * x) + 1) + start;
        // 1 - (y - start) / (end - start) = 2 ^ -10*x;
        // x = log (1 - (y - start) / (end - start)) / -10
    }

    public static float EaseOutExpoInverse(float start, float end, float xvalue)
    {
        return Mathf.Log(1 - (xvalue - start) / (end - start), 2) / -10;
    }
